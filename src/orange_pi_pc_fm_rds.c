 ...
 * tree and no compatible GPCLK/PWM FIFO pacing block, so this port exposes the
 * generated 228 kHz FM multiplex as a WAV stream for an external RF modulator
 * or SDR.  It keeps the RDS, stereo and control-pipe logic intact and avoids
 * poking Raspberry-specific /dev/vcio mailbox or BCM registers.
 */
...
static void usage(const char *argv0)
{
    fprintf(stderr,
        ...
        "Feed it to an SDR or an external VHF FM exciter; direct Raspberry Pi\n"
        "GPCLK transmission is not available on Allwinner H3.\n", argv0);
}
...
int main(int argc, char **argv)
{
    ...
    for (int i = 1; i < argc; i++) {
        ...
        if ((strcmp("-wav", arg) == 0 || strcmp("-audio", arg) == 0) && param) {
            ...
            fprintf(stderr, "Warning: %s is Raspberry Pi RF-hardware specific and is ignored on Orange Pi PC.\n", arg);
            if (param) i++;
        } else {
            usage(argv[0]);
            return 1;
    ...
    printf("Writing Orange Pi PC FM multiplex to %s at %d samples/s.\n", out_file, MPX_SAMPLE_RATE);
